...
async function fetchApi(apiWorker: Fetcher | null, domain: string, path: string): Promise<any> {
  try {
    let res: Response;
    if (apiWorker) {
      // Use service binding for direct Worker-to-Worker call (no subrequest through routes)
      res = await apiWorker.fetch(new Request(`https://${domain}${path}`, {
        headers: { Accept: 'application/json' },
      }));
    } else {
      // Fallback to external fetch
      res = await fetch(`https://${domain}${path}`, {
        headers: { Accept: 'application/json', 'User-Agent': 'SiliconBeest-OG/1.0' },
      });
    }
...
export async function handleOgRequest(url: URL, apiWorker?: Fetcher | null): Promise<Response | null> {
  const domain = url.hostname;
  const page = parsePageType(url.pathname);
  const worker = apiWorker ?? null;

  // Fetch instance info (used by all page types as fallback and for siteName)
  const instanceData = await fetchApi(worker, domain, '/api/v2/instance');
  ...
  if (page.type === 'status') {
    const statusData = await fetchApi(worker, domain, `/api/v1/statuses/${page.params.statusId}`);
    if (statusData) {
  ...
  if (page.type === 'profile') {
    ...
    const accountData = await fetchApi(worker, domain, `/api/v1/accounts/lookup?acct=${encodeURIComponent(acct)}`);
    if (accountData) {
