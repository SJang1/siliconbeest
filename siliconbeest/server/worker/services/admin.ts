...
export async function getAccountForModeration(
	...
	const row = await db.prepare('SELECT id, username, domain, uri FROM accounts WHERE id = ?1').bind(id).first();
 ...
 */
export async function sensitizeAccount(db: D1Database, id: string): Promise<void> {
	const now = new Date().toISOString();
	await db.prepare('UPDATE accounts SET sensitized_at = ?1 WHERE id = ?2').bind(now, id).run();
}

/**
 * Disable a user account (freeze).
 */
export async function disableAccount(db: D1Database, accountId: string): Promise<void> {
	await db.prepare('UPDATE users SET disabled = 1 WHERE account_id = ?1').bind(accountId).run();
}

/**
 * Silence an account (hide from public timelines).
 */
export async function silenceAccount(db: D1Database, id: string): Promise<void> {
	const now = new Date().toISOString();
	await db.prepare('UPDATE accounts SET silenced_at = ?1 WHERE id = ?2').bind(now, id).run();
}

/**
 * Suspend an account (full block). Returns the timestamp used.
 */
export async function suspendAccount(db: D1Database, id: string): Promise<string> {
	const now = new Date().toISOString();
	await db.prepare('UPDATE accounts SET suspended_at = ?1 WHERE id = ?2').bind(now, id).run();
	return now;
}
 ...
 */
export async function unsuspendAccount(db: D1Database, id: string): Promise<void> {
	await db.prepare('UPDATE accounts SET suspended_at = NULL WHERE id = ?1').bind(id).run();
}

/**
 * Remove silence from an account.
 */
export async function unsilenceAccount(db: D1Database, id: string): Promise<void> {
	await db.prepare('UPDATE accounts SET silenced_at = NULL WHERE id = ?1').bind(id).run();
}

/**
 * Re-enable a disabled (frozen) user account.
 */
export async function enableAccount(db: D1Database, accountId: string): Promise<void> {
	await db.prepare('UPDATE users SET disabled = 0 WHERE account_id = ?1').bind(accountId).run();
}

/**
 * Remove sensitized flag from an account.
 */
export async function unsensitizeAccount(db: D1Database, id: string): Promise<void> {
	await db.prepare('UPDATE accounts SET sensitized_at = NULL WHERE id = ?1').bind(id).run();
}
...
export async function resolveReport(
	...
	const now = new Date().toISOString();
	await db.prepare('UPDATE reports SET action_taken_at = ?1, action_taken_by_account_id = ?2 WHERE id = ?3')
...
export async function getUserEmailByAccountId(
	...
	return db.prepare('SELECT email, locale FROM users WHERE account_id = ?1')
		.bind(accountId)
...
export async function getAccountWithUser(
	...
	const account = await db.prepare('SELECT * FROM accounts WHERE id = ?1').bind(accountId).first();
	if (!account) throw new AppError(404, 'Record not found');

	const user = await db.prepare('SELECT * FROM users WHERE account_id = ?1').bind(accountId).first();
 ...
 */
export async function approveAccount(db: D1Database, accountId: string): Promise<void> {
	await db.prepare('UPDATE users SET approved = 1 WHERE account_id = ?1').bind(accountId).run();
}

/**
 * Reject (delete) a pending account. Removes both user and account records.
 */
export async function rejectAccount(db: D1Database, accountId: string): Promise<void> {
	await db.batch([
		db.prepare('DELETE FROM users WHERE account_id = ?1').bind(accountId),
...
export async function setAccountRole(
	...
	const user = await db.prepare('SELECT id, role FROM users WHERE account_id = ?1').bind(accountId).first();
	if (!user) throw new AppError(404, 'Record not found');

	await db.prepare('UPDATE users SET role = ?1, updated_at = ?2 WHERE account_id = ?3')
...
export async function listDomainBlocks(db: D1Database, limit: number): Promise<Record<string, unknown>[]> {
	const { results } = await db.prepare('SELECT * FROM domain_blocks ORDER BY id DESC LIMIT ?1').bind(limit).all();
	return (results || []) as Record<string, unknown>[];
}

export async function getDomainBlock(db: D1Database, id: string): Promise<Record<string, unknown>> {
	const row = await db.prepare('SELECT * FROM domain_blocks WHERE id = ?1').bind(id).first();
	if (!row) throw new AppError(404, 'Record not found');
	return row;
}
...
export async function createDomainBlock(
	...
	const existing = await db.prepare('SELECT id FROM domain_blocks WHERE domain = ?1').bind(data.domain).first();
...
export async function updateDomainBlock(
	...
	const existing = await db.prepare('SELECT * FROM domain_blocks WHERE id = ?1').bind(id).first();
...
export async function deleteDomainBlock(db: D1Database, id: string): Promise<string> {
	const existing = await db.prepare('SELECT * FROM domain_blocks WHERE id = ?1').bind(id).first();
	if (!existing) throw new AppError(404, 'Record not found');

	await db.prepare('DELETE FROM domain_blocks WHERE id = ?1').bind(id).run();
...
export async function listDomainAllows(db: D1Database, limit: number): Promise<Record<string, unknown>[]> {
	const { results } = await db.prepare('SELECT * FROM domain_allows ORDER BY id DESC LIMIT ?1').bind(limit).all();
	return (results || []) as Record<string, unknown>[];
}

export async function getDomainAllow(db: D1Database, id: string): Promise<Record<string, unknown>> {
	const row = await db.prepare('SELECT * FROM domain_allows WHERE id = ?1').bind(id).first();
	if (!row) throw new AppError(404, 'Record not found');
	return row;
}

export async function createDomainAllow(db: D1Database, domain: string): Promise<Record<string, unknown>> {
	const existing = await db.prepare('SELECT id FROM domain_allows WHERE domain = ?1').bind(domain).first();
...
export async function deleteDomainAllow(db: D1Database, id: string): Promise<void> {
	const existing = await db.prepare('SELECT * FROM domain_allows WHERE id = ?1').bind(id).first();
	if (!existing) throw new AppError(404, 'Record not found');

	await db.prepare('DELETE FROM domain_allows WHERE id = ?1').bind(id).run();
}
...
export async function listIpBlocks(db: D1Database, limit: number): Promise<Record<string, unknown>[]> {
	const { results } = await db.prepare('SELECT * FROM ip_blocks ORDER BY id DESC LIMIT ?1').bind(limit).all();
	return (results || []) as Record<string, unknown>[];
}

export async function getIpBlock(db: D1Database, id: string): Promise<Record<string, unknown>> {
	const row = await db.prepare('SELECT * FROM ip_blocks WHERE id = ?1').bind(id).first();
	if (!row) throw new AppError(404, 'Record not found');
	return row;
}
...
export async function updateIpBlock(
	...
	const existing = await db.prepare('SELECT * FROM ip_blocks WHERE id = ?1').bind(id).first();
	if (!existing) throw new AppError(404, 'Record not found');
...
export async function deleteIpBlock(db: D1Database, id: string): Promise<void> {
	const existing = await db.prepare('SELECT * FROM ip_blocks WHERE id = ?1').bind(id).first();
	if (!existing) throw new AppError(404, 'Record not found');

	await db.prepare('DELETE FROM ip_blocks WHERE id = ?1').bind(id).run();
}
...
export async function listEmailDomainBlocks(db: D1Database, limit: number): Promise<Record<string, unknown>[]> {
	const { results } = await db.prepare('SELECT * FROM email_domain_blocks ORDER BY id DESC LIMIT ?1').bind(limit).all();
	return (results || []) as Record<string, unknown>[];
}

export async function getEmailDomainBlock(db: D1Database, id: string): Promise<Record<string, unknown>> {
	const row = await db.prepare('SELECT * FROM email_domain_blocks WHERE id = ?1').bind(id).first();
	if (!row) throw new AppError(404, 'Record not found');
	return row;
}

export async function createEmailDomainBlock(db: D1Database, domain: string): Promise<Record<string, unknown>> {
	const existing = await db.prepare('SELECT id FROM email_domain_blocks WHERE domain = ?1').bind(domain).first();
...
export async function deleteEmailDomainBlock(db: D1Database, id: string): Promise<void> {
	const existing = await db.prepare('SELECT * FROM email_domain_blocks WHERE id = ?1').bind(id).first();
	if (!existing) throw new AppError(404, 'Record not found');

	await db.prepare('DELETE FROM email_domain_blocks WHERE id = ?1').bind(id).run();
}
...
export async function listCustomEmojis(db: D1Database, domain: string): Promise<Record<string, unknown>[]> {
	const { results } = await db.prepare(
...
export async function getCustomEmoji(db: D1Database, id: string): Promise<Record<string, unknown>> {
	const row = await db.prepare('SELECT * FROM custom_emojis WHERE id = ?1').bind(id).first();
	if (!row) throw new AppError(404, 'Record not found');
	return row;
}
...
export async function createCustomEmoji(
	...
	const row = await db.prepare('SELECT * FROM custom_emojis WHERE id = ?1').bind(data.id).first();
	return row!;
}
...
export async function updateCustomEmoji(
	...
	const existing = await db.prepare('SELECT * FROM custom_emojis WHERE id = ?1').bind(id).first();
	if (!existing) throw new AppError(404, 'Record not found');
...
export async function deleteCustomEmoji(db: D1Database, id: string): Promise<string | null> {
	const existing = await db.prepare('SELECT * FROM custom_emojis WHERE id = ?1').bind(id).first();
	if (!existing) throw new AppError(404, 'Record not found');

	await db.prepare('DELETE FROM custom_emojis WHERE id = ?1').bind(id).run();
...
export async function listAnnouncements(db: D1Database): Promise<Record<string, unknown>[]> {
	const { results } = await db.prepare('SELECT * FROM announcements ORDER BY created_at DESC').all();
	return (results || []) as Record<string, unknown>[];
}

export async function getAnnouncement(db: D1Database, id: string): Promise<Record<string, unknown>> {
	const row = await db.prepare('SELECT * FROM announcements WHERE id = ?1').bind(id).first();
	if (!row) throw new AppError(404, 'Record not found');
	return row;
}
...
export async function updateAnnouncement(
	...
	const existing = await db.prepare('SELECT * FROM announcements WHERE id = ?1').bind(id).first();
	if (!existing) throw new AppError(404, 'Record not found');
...
export async function deleteAnnouncement(db: D1Database, id: string): Promise<void> {
	...
	await db.batch([
		db.prepare('DELETE FROM announcement_dismissals WHERE announcement_id = ?1').bind(id),
...
export async function listRelays(db: D1Database): Promise<RelayRow[]> {
	const { results } = await db.prepare('SELECT * FROM relays ORDER BY created_at DESC').all<RelayRow>();
	return results || [];
}

export async function getRelay(db: D1Database, id: string): Promise<RelayRow> {
	const row = await db.prepare('SELECT * FROM relays WHERE id = ?1').bind(id).first<RelayRow>();
	if (!row) throw new AppError(404, 'Record not found');
	return row;
}

export async function checkRelayExists(db: D1Database, inboxUrl: string): Promise<boolean> {
	const existing = await db.prepare('SELECT id FROM relays WHERE inbox_url = ?1').bind(inboxUrl).first();
	return !!existing;
}
...
export async function createRelay(
	...
	return (await db.prepare('SELECT * FROM relays WHERE id = ?1').bind(id).first<RelayRow>())!;
}

export async function deleteRelay(db: D1Database, id: string): Promise<void> {
	await db.prepare('DELETE FROM relays WHERE id = ?1').bind(id).run();
}

/**
 * Ensure the instance actor keypair exists and return it.
 */
export async function getInstanceActorKey(db: D1Database, domain: string, title?: string): Promise<{
	public_key: string;
	private_key: string;
	key_id: string;
}> {
	...
	).bind(domain, title || 'SiliconBeest', `https://${domain}/actor`, `https://${domain}/about`, now).run();
...
export async function getInstance(db: D1Database, domain: string): Promise<Record<string, unknown> | null> {
...
export async function getFederationStats(db: D1Database): Promise<{
	...
	const [totalInstances, activeInstances, unreachableInstances, remoteAccounts] = await Promise.all([
		db.prepare('SELECT COUNT(*) AS cnt FROM instances').first<{ cnt: number }>(),
		db.prepare(
		...
		db.prepare('SELECT COUNT(*) AS cnt FROM accounts WHERE domain IS NOT NULL').first<{ cnt: number }>(),
	]);
