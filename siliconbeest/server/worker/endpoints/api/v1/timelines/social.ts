// Social timeline: the viewer's home timeline merged with the local
// public timeline. Serialization mirrors home.ts.
...
app.get('/', authRequired, requireScope('read:statuses'), async (c) => {
  ...
  const allRows = await getSocialTimeline(account.id, {
    maxId: pag.maxId,
    sinceId: pag.sinceId,
    minId: pag.minId,
    limit: pag.limit,
  ...
  const baseUrl = `https://${env.INSTANCE_DOMAIN}/api/v1/timelines/social`;
