...
app.get('/', authRequired, requireScope('read:favourites'), async (c) => {
  ...
  const { whereClause, orderClause, limitValue, params } = buildPaginationQuery(pag, 'f.id');
  ...
  const sql = `
    SELECT f.id AS f_id, s.*, a.id AS a_id, a.username AS a_username, a.domain AS a_domain,
