...
app.get('/', async (c) => {
	...
	const results = await listEmailDomainBlocks(c.env.DB, limit);
 ...
 */
app.get('/:id', async (c) => {
	const row = await getEmailDomainBlock(c.env.DB, c.req.param('id'));
...
app.post('/', async (c) => {
	...
	const row = await createEmailDomainBlock(c.env.DB, body.domain);
...
app.delete('/:id', async (c) => {
	await deleteEmailDomainBlock(c.env.DB, c.req.param('id'));
	return c.json({}, 200);
});
